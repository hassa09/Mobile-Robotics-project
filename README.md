...
$ git clone https://github.com/hassa09/Mobile-Robotics-project.git
$ cd .. && colcon build
...
## Procedure
### 1. Installation
Firstly, installed the [ROS 2 Humble Hawksbil](https://docs.ros.org/en/humble/Installation/Ubuntu-Install-Debians.html#install-ros-2-packages)) distribution of **ROS2**.
...
More specifically, I used the **nav2_bringup** simulation,then added the STVL layers as plugins to the **nav2_params.yaml**.
...
### 4. Mapping with SLAM
I chose the SLAM toolbox to map our environments as it was the quickest option available to us. The toolbox was installed through apt, following the nav2 documentation ["Navigating While Mapping"](https://navigation.ros.org/tutorials/docs/navigation2_with_slam.html).  
<br>
...
Then, using [teleop_twist_keyboard](http://wiki.ros.org/teleop_twist_keyboard), we explored the entire environment whilst mapping it, and then saved the map through the map_saver_cli node, as detailed in the nav2 documentation ["Map Saver / Saver"](https://navigation.ros.org/configuration/packages/configuring-map-server.html).
<br>
### Navigation Package
